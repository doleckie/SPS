    ...
    for token in code:
        ...
        try:
            ...
            if token == "add":
                ...
                defn = lookup(str(token)) #check if token is the name of a definition and set the value of the definition to defn (lookup returns False if the name is not in the dictStack
                debug("Definition look up returned: " + str(defn))
                if defn != False: #if the defn is not False
                    ...
                    if type(defn) == type([]):
                        ...
                        opPush(defn)
                else: #else the token is something not covered above so push onto the opStack
                    opPush(token)
